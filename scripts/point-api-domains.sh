#!/usr/bin/env bash
#
# Point the live API hostnames at a Worker.
#
...
# per hostname, because a hostname can only belong to one Worker — that gap is
# the only downtime.
#
...
HOSTS=(api.ensideas.com api.instantens.com)
...
import sys, json
for r in json.load(sys.stdin).get('result', []):
    if r['hostname'] in ('api.ensideas.com', 'api.instantens.com'):
